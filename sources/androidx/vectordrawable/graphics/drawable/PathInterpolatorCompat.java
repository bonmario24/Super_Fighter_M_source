...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (((double) Math.abs(this.f40mX[0])) > 1.0E-5d || ((double) Math.abs(this.f41mY[0])) > 1.0E-5d || ((double) Math.abs(this.f40mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f41mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
        ...
        while (i2 < numPoints) {
            ...
            float x = this.f40mX[componentIndex];
            if (x < prevX) {
    ...
    public float getInterpolation(float t) {
        ...
        float xRange = this.f40mX[endIndex] - this.f40mX[startIndex];
        if (xRange == 0.0f) {
            return this.f41mY[startIndex];
        }
        float startY = this.f41mY[startIndex];
        return ((this.f41mY[endIndex] - startY) * ((t - this.f40mX[startIndex]) / xRange)) + startY;
    }
}
