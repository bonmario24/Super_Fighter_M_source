...
class FindAddress {
    ...
    private static boolean checkHouseNumber(String houseNumber) {
        ...
        switch (num % 10) {
            case 1:
                return suffix.equals(num % 100 == 11 ? Language.f821th : UserDataStore.STATE);
            case 2:
                return suffix.equals(num % 100 == 12 ? Language.f821th : "nd");
            case 3:
                return suffix.equals(num % 100 == 13 ? Language.f821th : "rd");
            default:
                return suffix.equals(Language.f821th);
        }
    }
                ...
                if (matcher.find(it)) {
                    if (matcher.end() - matcher.start() <= 25) {
                        while (it < matcher.start()) {
                            int it3 = it + 1;
                            if (f35NL.indexOf(content.charAt(it)) != -1) {
                                numLines++;
                                it = it3;
                            } else {
                                it = it3;
