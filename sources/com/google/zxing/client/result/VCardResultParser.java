            ...
            if (i < 0) {
                ...
                if (quotedPrintable) {
                    ...
                    if (parseFieldDivider) {
                        element = UNESCAPED_SEMICOLONS.matcher(element).replaceAll(ShellAdbUtil.COMMAND_LINE_END).trim();
                    }
                } else {
                    if (parseFieldDivider) {
                        element2 = UNESCAPED_SEMICOLONS.matcher(element2).replaceAll(ShellAdbUtil.COMMAND_LINE_END).trim();
                    }
                    element = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(element2).replaceAll("")).replaceAll(ShellAdbUtil.COMMAND_LINE_END)).replaceAll("$1");
                }
                if (metadata == null) {
