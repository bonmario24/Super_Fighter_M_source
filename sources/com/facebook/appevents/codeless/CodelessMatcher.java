...
class CodelessMatcher {
    ...
    protected static class ViewMatcher implements ViewTreeObserver.OnGlobalLayoutListener, ViewTreeObserver.OnScrollChangedListener, Runnable {
        ...
        private static boolean isTheSameView(View targetView, PathComponent pathElement, int index) {
            ...
            if ((pathElement.matchBitmask & PathComponent.MatchBitmaskType.ID.getValue()) > 0 && pathElement.f832id != targetView.getId()) {
                return false;
            }
        ...
        private void attachListener(MatchedView matchedView, View rootView2, EventBinding mapping) {
            boolean delegateExists;
            boolean isCodelessDelegate;
            ...
            if (mapping != null) {
                try {
                    ...
                    if (view != null) {
                        ...
                        if (RCTRootView != null && ViewHierarchy.isRCTButton(view, RCTRootView)) {
                            attachRCTListener(matchedView, rootView2, RCTRootView, mapping);
                            ...
                            if (!delegateExists || !(existingDelegate instanceof CodelessLoggingEventListener.AutoLoggingAccessibilityDelegate)) {
                                isCodelessDelegate = false;
                            } else {
                                isCodelessDelegate = true;
                            }
                            if (!isCodelessDelegate || !((CodelessLoggingEventListener.AutoLoggingAccessibilityDelegate) existingDelegate).getSupportCodelessLogging()) {
                                delegateSupportCodelessLogging = false;
                            }
                            if (this.delegateMap.containsKey(mapKey)) {
                                return;
                            }
                            if (!delegateExists || !isCodelessDelegate || !delegateSupportCodelessLogging) {
        ...
        private void attachRCTListener(MatchedView matchedView, View rootView2, View RCTRootView, EventBinding mapping) {
            View view;
            boolean listenerExists;
            boolean isCodelessListener;
            boolean listenerSupportCodelessLogging = true;
            if (mapping != null && (view = matchedView.getView()) != null && ViewHierarchy.isRCTButton(view, RCTRootView)) {
                String mapKey = matchedView.getViewMapKey();
                ...
                if (!listenerExists || !(existingListener instanceof RCTCodelessLoggingEventListener.AutoLoggingOnTouchListener)) {
                    isCodelessListener = false;
                } else {
                    isCodelessListener = true;
                }
                if (!isCodelessListener || !((RCTCodelessLoggingEventListener.AutoLoggingOnTouchListener) existingListener).getSupportCodelessLogging()) {
                    listenerSupportCodelessLogging = false;
                }
                if (this.delegateMap.containsKey(mapKey)) {
                    return;
                }
                if (!listenerExists || !isCodelessListener || !listenerSupportCodelessLogging) {
                    view.setOnTouchListener(RCTCodelessLoggingEventListener.getOnTouchListener(mapping, rootView2, view));
                    this.delegateMap.put(mapKey, mapping.getEventName());
                }
            }
        }
